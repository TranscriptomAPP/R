...
qc <-as.matrix(qc)
qc <- c(qc[1], qc[2],qc[3])
pr <- c((qc[1]/qc[1])*100, (qc[2]/qc[1])*100,(qc[3]/qc[1])*100)
pr <- round (pr, digits=2)

#opens pdf for printing
pdf(file=paste(rn, ".pdf", sep = ""))
par(mar=c(9, 4, 4, 2)+0.1)
  coords <- barplot(qc, main=paste("Trimming results", rn, sep =" - "),names.arg=c("Raw Reads", "Trimmed", "Dropped"), ylab="Read Pair Count", ylim=c(0,max(qc)*1.2), las=2, las=3)
